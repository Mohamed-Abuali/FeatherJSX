...
*   **Solution:** Diagnosed a classic edge case in the keyed reconciliation logic.
    *   **Cause:** `null` keys were being treated as valid keys. This caused multiple elements (which didn't have explicit keys) to "collide" in the key map, all mapping to the same `null` key entry.
    *   **Consequence:** The reconciler tried to reuse/move the *same* DOM element multiple times for different virtual nodes, resulting in chaotic duplication.
    *   **Fix:** Strict validation to ensure only defined, non-null keys are added to the `oldKeysMap`.
